...
LAYER=$(cat team.tcl | grep "TARGET LAYER" | cut -d " " -f 4)
...
for i in `seq 0 9`; do
    ...
    cd $src
    make sim_BEHAV_${1}_TARGET_LAYER_${LAYER}_${img} -j > $dst/accuracy_comp.log
    cd $dst
...
echo "Layer ${LAYER} accuracy is ${accuracy}%."
