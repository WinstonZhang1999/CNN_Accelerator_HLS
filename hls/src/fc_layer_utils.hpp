...
#ifndef __FC_LAYER_UTILS_HPP__
#define __FC_LAYER_UTILS_HPP__

inline void fc_layer::load_compute_handshake(void)
{
    {
...
inline void fc_layer::compute_load_handshake(void)
{
    {
...
inline void fc_layer::compute_store_handshake(void)
{
    {
...
inline void fc_layer::store_compute_handshake(void)
{
    {
...
void fc_layer::wait_for_config(void)
{
    bool go = false;
    do {
        go  = cmd_sig.read()             == ACCELERATOR_CMD_GO;
        go &= num_w_cols_sig.read()      != 0;
        go &= num_w_rows_sig.read()      != 0;
...
void fc_layer::irq_compute(void)
{
...
void fc_layer::process_done(void)
{
    do { wait(); }
...
template<class T>
void fc_layer::load_data(plm_data32_1w1r_t<T, PLM_SIZE> &array,
...
void fc_layer::store_data(plm_data32_1w1r_t<FPDATA_WORD, PLM_SIZE> &array,
                          uint32_t base_addr, uint32_t size)
{
    uint32_t index = 0;
    uint32_t beat_id = 1;
    ...
    for (index = size; index > 0; )
    {
        ...
        for (; j < beats; ++j)
        {
            ...
            // TODO: handle different DATA WIDTH
            wchan.data = array.port2[0][mem_index];
